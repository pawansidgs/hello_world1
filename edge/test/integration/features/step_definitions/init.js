...
var environment = config['hello_world1'].env;
...
console.log('&hello_world1 api: [' +  domains[environment] + ', ' + config['hello_world1'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hello_world1'].app, config['hello_world1'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hello_world1'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
